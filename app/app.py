...
# pending_transactions에 transaction 추가
@app.route('/transaction', methods=['POST'])
def create_transaction():
    new_transaction = request.get_json()
    block_index = bitcoin.add_transaction_to_pending_transactions(
        new_transaction)
...
def mine():
    ...
    block_hash = bitcoin.hash_block(
        previous_block_hash, current_block_data, nonce)
    new_block = bitcoin.create_new_block(
        nonce, previous_block_hash, block_hash)
    ...
    for network_node_url in bitcoin.network_nodes:
        ...
        res = requests.post(network_node_url +
                            '/receive-new-block', json=request_options)
    ...
    requests.post(bitcoin.current_node_url +
                  '/transaction/broadcast', json=request_options)
...
def register_and_broadcast_node():
    ...
    reg_nodes_promises = []
    for network_node_url in bitcoin.network_nodes:
        # 새로운 노드를 연결하는 요청 받은 노드가 원래 연결되어 있던 노드에게 새로운 노드를 등록하는 요청 보내는 API 호출
        response = requests.post(
            f"{network_node_url}/register-node", json={'newNodeUrl': new_node_url})
...
# 새로운 노드를 연결하는 요청 받은 노드가 원래 연결되어 있던 노드에게 새로운 노드를 등록하는 요청 보내는 API
...
# 새로운 노드를 추가한 뒤 전체 노드 정보를 새로 연결되는 노드에게 주는 API
